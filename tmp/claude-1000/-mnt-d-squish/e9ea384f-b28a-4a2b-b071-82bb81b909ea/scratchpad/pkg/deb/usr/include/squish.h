 ...
 * Add or remove members without re-packing the whole archive. Kept members
 * are copied *verbatim* — their already-compressed blocks are moved byte for
 * byte — so only files being added are ever compressed. Each call rebuilds the
 * archive into a sibling temp file and atomically replaces the original, so a
 * failure (I/O error, full disk) leaves the existing archive untouched. The
 * rebuilt archive is byte-identical to one squish_archive_create would produce
 * from the same resulting tree.
 *
 * Both operate on a packed (tree) archive; an archive with the SINGLE flag (a
 * lone file/blob, e.g. from squish_compress_file) returns SQUISH_E_PARAM. */

/* `flags` for squish_archive_add. */
#define SQUISH_ADD_KEEP_EXISTING 1u   /* skip files already present (don't clobber) */

/* Add (merge) the filesystem entry at src_path — a regular file, or a directory
 * added recursively — into the archive at archive_path, under the archive-
 * relative arc_path (UTF-8, '/'-separated, no trailing slash, e.g.
 * "docs/readme.txt"). Missing parent directories of arc_path become directory
 * members. A directory merges into an existing one of the same name: files it
 * contains are added, existing files it does not mention are left in place, and
 * a file that collides with an existing file is overwritten — or, with
 * SQUISH_ADD_KEEP_EXISTING in `flags`, kept. (A file colliding with a directory
 * of the same path, or vice versa, is SQUISH_E_PARAM.) Added files are
 * compressed at the archive's existing block size using `nthreads` (0 = all
 * cores). `cb` (may be NULL) reports uncompressed bytes processed. Returns
 * SQUISH_OK or SQUISH_E_PARAM (bad path, type clash, or a SINGLE archive) /
 * E_IO / E_NOMEM / E_FORMAT / E_TOOBIG. */
...
/* Remove members from the archive at archive_path: every member whose path is
 * exactly one of paths[0..npaths) or lies beneath such a path (its subtree). A
 * trailing '/' on a path is tolerated. Removing every member yields a valid
 * empty archive. `cb` (may be NULL) reports uncompressed bytes kept. Returns
 * SQUISH_OK, SQUISH_E_PARAM (no paths, or a SINGLE archive), SQUISH_E_FORMAT if
 * no member matches any given path (the archive is left unchanged), or
 * SQUISH_E_IO / E_NOMEM. */
